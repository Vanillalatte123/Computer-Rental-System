    ...
    <div class="row mt-5">
        <div class="col">
            <?php
                ...
                $query = "SELECT *,op.id as orderID FROM `orderproduct` AS op LEFT JOIN `product` AS pr ON pr.Id = op.Id WHERE op.Id =$id";
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo '<div class="card bg-white p-3" style="width: 500px;border-radius: 20px;">
                            <div class="card-body text-center">
                                <h4 class="card-title mb-4" style="text-align:center; font-weight:bold;">'. $row['productModel'] .'</h4>
                                <img src="'.$row["productImage"].'" style="width:300px; height:300px;">
        ...
        <div class="col">
            ...
            <div class="card bg-white p-3" style="width: 700px;border-radius: 20px;">
                <div class="card-body">
                    <h4 class="card-title" style="text-align:center; font-weight:bold;">Computer Rental</h4>

                    <!-- Added form elements from Bootstrap (form-label, form-control) -->
                    <div class="container" style="margin-top:25px">
                                <!-- Booth Number -->
                                <div class="col-sm-12" style="margin-top:20px">
                                    <label for="booth_number" class="form-label fw-bold">Your Order number is: </label>
                                    <?php 
                                        $query = "SELECT *,op.id as orderID FROM `orderproduct` AS op LEFT JOIN `product` AS pr ON pr.Id = op.Id WHERE op.Id =$id";
                                        $result = mysqli_query($dbc, $query);
                                        $num = mysqli_num_rows($result);
                                        $row = mysqli_fetch_assoc($result);
                                        $booth_number = $row['orderID'];
                                        echo '<input id="booth_number" type="text" name="booth_number" class="form-control" value="'.  $booth_number .'" disabled>';
                                    ?>
                                </div>
                                <!-- Booth Price -->
                                <div class="col-sm-12" style="margin-top:20px">
                                    <label for="booth_price" class="form-label fw-bold">Product Price (RM):</label>
                                    <?php echo '<input id="booth_price" type="text" name="booth_price" class="form-control" value="'.  $row['totalPrice'] .'" disabled>';?>
                                </div>
                                <!-- Status -->
                                <div class="col-sm-12" style="margin-top:20px">
                            ...
                            <!-- Back Button -->
                            <?php echo '<form action="myOrder.php" method="POST" style="text-align: center;">'?>
